...
public abstract class MessageRouter {
	...
	public boolean hasMessage(String id) {
		...
		if (this.messages.containsKey(id)) {
			...
			int coded_num = 0;
			StringTokenizer st = new StringTokenizer(id, "M");
			String codedid = st.nextToken();
			String[] coded_ids = new String[st.countTokens()];
			int k = 0;
			int[] coded_coef = new int[st.countTokens()];
			StringTokenizer st2 = new StringTokenizer(id, "T");
			String codecoef = st2.nextToken();
			if (firstchar == 'C') {
				while (st.hasMoreTokens()) {
					coded_ids[k] = "M" + st.nextToken();
					k++;
				}
				int l = 0;
				while ((st2.hasMoreTokens()) && (l <= (k - 1))) {
					coded_coef[l] = (int) Integer.parseInt(st2.nextToken());
					l++;
				}
				idGeneration=Integer.parseInt(coded_ids[0].substring(1,coded_ids[0].length()))/generation;
			}
			else{
				idGeneration=Integer.parseInt(id.substring(1, id.length()))/generation;
			}
			Vector<Message> vmessage = new Vector<Message>();
			int size=0;
			for(Message mn : getMessageCollection()){
				if(mn.getGeneration()==idGeneration){
					size++;
				}
			}
			int[][] C =new int[size+1][generation];
			Arrays.fill(C[0],0);
			if(id.charAt(0)=='C'){
				for(int j=0;j<coded_ids.length;j++){				
					int temp = Integer.parseInt(coded_ids[j].substring(1,coded_ids[j].length()));
					temp = temp%generation; 
					C[0][temp]=coded_coef[j];
				}				
			}
			else{
				int temp=Integer.parseInt(id.substring(1, id.length()));
				temp=temp%generation;
				C[0][temp]=1;				
			}
			k=1;
			for (Message mn : getMessageCollection()) {				
				String[] coded_id = mn.getCodedid();
				if ((mn.is_coded()) && (firstchar == 'M')) {
					host1 = mn.getTo();
					for (int i = 0; i < coded_id.length; i++) {
						if (id.equals(coded_id[i])) {
							if ((!host1.equals(this.host)) && (this.host.toString().charAt(0)!='W')) {
								hasmessage = true;
								return hasmessage;
							}
						}
				...
				if(mn.is_coded() && mn.getGeneration()==idGeneration){
					Arrays.fill(C[k],0);
					for(int j=0;j<coded_id.length;j++){		
//						System.out.println("oded_id.length="+coded_id.length);
//						System.out.println(coded_id[j]);
						int temp = Integer.parseInt(coded_id[j].substring(1,coded_id[j].length()));
						temp = temp%generation; 
						C[k][temp]=mn.getCoding_Coef()[j];
					}
					k++;
				}
				else if((mn.getGeneration()==idGeneration)&&(!mn.is_coded()) ){
					Arrays.fill(C[k],0);
					int temp=Integer.parseInt(mn.getId().substring(1, mn.getId().length()));
					temp=temp%5;
					C[k][temp]=1;
					k++;
				}
			}
			if (Rank.Array_rank(C) != size+1) {
				hasmessage = true;
			}
		}
		return hasmessage;
	...
	protected boolean isDeliveredMessage(Message m) {
		boolean is_delivered = this.deliveredMessages.containsKey(m.getId());
		if (m.is_coded()) {
			int j = 0;
			String[] id = m.getCodedid();
			for (int i = 0; i < id.length; i++) {
				if (this.deliveredMessages.containsKey(id[i])) {
					j++;
				}
			}
			if (m.getCodedid().length == j) {
				is_delivered = true;
			}
		}
		return (is_delivered);
...
//		if(m.getId().equals("M30")) {
//			System.out.println("ganniM30 :");
//			for(Message mn : getMessageCollection()){
...
//
//		}
//		if(m.getId().equals("M30")&&host.toString().equals("H28")){
//			System.out.println("receiveMessage function:");
//			for(Message mn: this.getMessageCollection()){
//				System.out.print(" "+mn.getId());
//			}
//			System.out.println();
//		}
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		boolean can_decode = false;
		Vector<Message> bMessage = new Vector<Message>();
		...
		if ((isFirstDelivery) && (!aMessage.is_coded())) {
		...
		if (aMessage.is_coded() && isFinalRecipient) {
			 Set set = this.deliveredMessages.entrySet();
			 int deliverNum=0,messagesNum=0;
			 for(Iterator iter = set.iterator(); iter.hasNext();)
			 {
				 Map.Entry entry = (Map.Entry)iter.next();
				 String key = (String)entry.getKey();
				 Message value = (Message) entry.getValue();
				 if(value.getGeneration()==aMessage.getGeneration()){
					 bMessage.add(value);
					 deliverNum++;
				 }
			 }
			 for(Message mn: this.getMessageCollection()){
				 if(mn.getGeneration()==aMessage.getGeneration()){
					 bMessage.add(mn);
					 messagesNum++;
				 }
			 }
			 bMessage.add(aMessage);
			 Map<Message,Integer> messageAndPosi;			
			 if(bMessage.size()>=1){
				 messageAndPosi = Network_coding.DeCodeFei(bMessage);				 
				 Set set1 = messageAndPosi.entrySet();	
				 for(Iterator iter = set1.iterator(); iter.hasNext();)
				 {
					 Map.Entry entry = (Map.Entry)iter.next();		   
					 Message mn = (Message)entry.getKey();
					 int value = (int)entry.getValue();
					 boolean isFirstDelivery2 = !isDeliveredMessage(mn);
					 if (isFirstDelivery2) {
						 can_decode=true;							
...
//							}
						...
						for (MessageListener ml : this.mListeners) {
							ml.messageTransferred(mn, from, this.host,isFirstDelivery2);
						}
						if(value>=deliverNum && value<=deliverNum+messagesNum-1){
							deleteMessage((String) bMessage.get(value).getId(), false);
						}
					 }
				 }
				 if (!can_decode) {
	 ...
	 */
	protected void addToMessages1(Message m, boolean newMessage) {
		
		Vector<Message> v = new Vector<Message>();
		Vector<Message> c = new Vector<Message>();
		Vector<Message> v1 = new Vector<Message>();
		Vector<Message> m1 = new Vector<Message>();
		boolean can_code;
		can_code = false;
		DTNHost h1 = m.getTo();
//		if(m.getId().equals("M3")){
//			System.out.println("M3来到"+host.toString()+"啦:\ttime:"+SimClock.getTime());
//		}
		//System.out.println("host:"+host.toString()+"get message "+m.getId()+" time:"+SimClock.getTime());
		if (!(h1.equals(this.host))) {
			boolean hasmessage = false;
			boolean flag = false;
			for (Message mn : getMessageCollection()) {
				DTNHost h = mn.getTo();
				flag = false;
				if (mn.getGeneration() == m.getGeneration()) {
//					if(m.getId().equals("M45")){
	//					System.out.println(" "+mn.getId());
//					}
					if (mn.is_coded()) {
						c.add(mn);//c中存放已经编码过的mn
					} 
					else {
						if (m.is_coded()) {
							String[] coded_ids = m.getCodedid();
							for (int l = 0; l < coded_ids.length; l++) {
								if (mn.getId().equals(coded_ids[l])) {
									hasmessage = true;
									flag = true;
									m1.add(mn) ;
								}
						...
						if (!flag) {
							v.add(mn);//v中存放没有编码的mn,且mn不是m的源码包
							can_code = true;
						}
					}
				}
			...
			//System.out.println();
			{//此时v中存放所有未编码的mn，c中存放所有编码过的mn
				if ((v.size() == 0) && (c.size() == 0)) {//这个节点存储的信息中没有与到来的m是同一代的					
					this.messages.put(m.getId(), m);
//					if(m.getId().equals("M30") &&this.host.toString().equals("H28") )
//						System.out.println(this.host.toString()+"addM30:");
				} 
				else {
					if (!m.is_coded()) {
						boolean cannotcode = false;
...
//									System.out.println(v1.get(y).getId()+"hehe");
//								}
								Message newm = Network_coding.Code(v1);
								can_code = true;
								this.messages.put(newm.getId(), newm);
								deleteMessage((String) c.get(i).getId(), false);
							}						
						for (int o = 0; o < v.size(); o++) {
								deleteMessage((String) v.get(o).getId(), false);
							}						
						if ((c.size() == 0) && (can_code)) {
							v.add(m);	
//							for(int y=0;y<v.size();y++){
//							System.out.println(v.get(y).getId()+"hehe");
//						}
							for(int i=0;i<v.size()-1;i++){
								Message newm = Network_coding.Code(v);
								this.messages.put(newm.getId(), newm);
							}
						}
					}
						...
						for (int i = 0; i < v.size(); i++) {
							v1.add(v.get(i));
							deleteMessage((String) v.get(i).getId(), false);
						}
						if (can_code) {
							v1.add(m);
						...
						if (m.is_coded() && m1.size()>0) {
							for(Message mn1: m1){
								Message mnew_refresh = Network_coding.Refresh(m, mn1);
								deleteMessage(mn1.getId(), false);
								this.messages.put(mnew_refresh.getId(), mnew_refresh);
								add=false;
							}
						} 
						if(add){
	...
	protected void addToMessages(Message m, boolean newMessage) {
		Vector<Message> v = new Vector<Message>();
		Vector<Message> c = new Vector<Message>();
		Vector<Message> c2 = new Vector<Message>();
		Vector<Message> v1 = new Vector<Message>();
		Vector<Message> m1 = new Vector<Message>();
		boolean can_code= false;
		...
		//m is code 
			...
			for(int i=0;i<c2.size();i++){
				...
				v1.add(c2.get(i));
				Message newm = Network_coding.Code(v1);
				deleteMessage((String) c2.get(i).getId(), false);			
				this.messages.put(newm.getId(), newm);		
			}
		}	
	}
...
//		if(id.equals("M30")&&this.host.toString().equals("H28")){
//			System.out.println(this.host.toString()+"messageAborted:");
//			for(Message mn: this.getMessageCollection()){
//				System.out.print(" "+mn.getId());
//			}
//			System.out.println();
