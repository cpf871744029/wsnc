...
public abstract class ActiveRouter extends MessageRouter {
	...
	public boolean requestDeliverableMessages(Connection con) {
		...
		boolean flag = false;
		for (Message m : temp) {
			if (other == m.getTo()|| (other.toString().charAt(0)=='W'&&this.getHost().toString().charAt(0)=='H')) {
...
//		if(this.getHost().toString().equals("H28")&&m.getId().equals("M30")){
//			System.out.println("checkReceiving:H28,M30:\ttime:"+SimClock.getTime()+"\tfrom node:"+from.toString());
//			 
//		}
//			System.out.print("heheH28:\t"+m.getId()+"\t");
...
//		if(this.getHost().toString().equals("H28")){
//			System.out.print("heheH28:\t"+m.getId()+"\t");
...
//	}
	...
	protected List<Tuple<Message, Connection>> getMessagesForConnected() {
		...
		for (Message m : getMessageCollection()) {
			for (Connection con : getConnections()) {
				...
				if (m.getTo() == to || (this.getHost().toString().charAt(0)=='H'&&to.toString().charAt(0)=='W')) {
	...
	protected Tuple<Message, Connection> tryMessagesForConnected(
		...
		boolean flag=false;
		Tuple<Message, Connection> temp=null;
	...
	protected Message tryAllMessages(Connection con, List<Message> messages) {
		...
		for (Message m : messages) {
			...
			for (Message mn : getMessageCollection()) {
				if (mn.getGeneration() == m.getGeneration() && !mn.equals(m)) {
					if (mn.is_coded()) {
			...
			if((v.size()==0 && c.size() == 0) && !m.is_coded()){				 
				continue;
			} 
